...
## 2024-12-31 - [URL Sanitization for XSS Prevention]
**Vulnerability:** XSS via `javascript:` and other dangerous URI schemes in `src`, `href`, or `background-image` attributes.
**Learning:** `escapeHTML` is insufficient for attributes that expect URLs, as `javascript:alert(1)` is a valid escaped string.
**Prevention:** Use a `sanitizeURL` utility that whitelists safe protocols (`http`, `https`, `data:image`, and relative paths) and strips characters like `< > " ' ( )` to prevent attribute breakout and execution.
