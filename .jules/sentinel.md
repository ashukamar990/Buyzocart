...
## 2025-01-24 - [Harden Product Cards against XSS]
**Vulnerability:** XSS via product names and badges in `createProductCard`.
**Learning:** Reusable product card components that use `innerHTML` for performance or convenience are prime targets for XSS if seller-provided data (like product names) is not sanitized.
**Prevention:** Implement a global `escapeHTML` utility and ensure all dynamic strings injected into `innerHTML` are wrapped in it.
