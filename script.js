    ...
    function initProductDetailGallery(product) {
      ...
      if (zoomBtn) {
        zoomBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          pauseSlide();
          openZoomModal(currentProductImages[currentImageIndex]);
          resumeSlideAfterDelay();
        });
      }
      startAutoSlide();
  ...
  // ACCOUNT PAGE
    ...
    // Global state
    let accCurrentUser = null;
    let accEditingAddressId = null;
    let accAlertCallback = null;

    
    });

    function accInitTheme() {
      const savedTheme = localStorage.getItem('theme') || 'light';
      document.documentElement.setAttribute('data-theme', savedTheme);
      if (savedTheme === 'dark') {
    ...
    function accSetupEventListeners() {
      // Theme
      document.getElementById('themeToggle')?.addEventListener('click', accToggleTheme);
      ...
      // Profile
      document.getElementById('saveProfile').addEventListener('click', accSaveProfile);
      document.getElementById('changePasswordBtn').addEventListener('click', () => accOpenModal('changePasswordModal'));
      document.getElementById('deleteAccountBtn').addEventListener('click', accShowDeleteConfirmation);
      document.getElementById('accountLogoutBtn').addEventListener('click', accShowLogoutConfirmation);
      document.getElementById('viewLoginActivity').addEventListener('click', accShowLoginActivity);
      
      // Settings
      document.getElementById('saveSettings').addEventListener('click', accSaveSettings);

      // Address
      document.getElementById('addNewAddress').addEventListener('click', accShowAddressForm);
      document.getElementById('cancelAddAddress').addEventListener('click', accHideAddressForm);
      document.getElementById('saveAddress').addEventListener('click', accSaveAddress);

      // Password modal
      document.getElementById('cancelChangePassword').addEventListener('click', () => accCloseModal('changePasswordModal'));
      document.getElementById('sendResetLink').addEventListener('click', accSendPasswordReset);

      // Alert modal
      document.getElementById('alertCancelBtn').addEventListener('click', () => accCloseModal('alertModal'));
      document.getElementById('alertConfirmBtn').addEventListener('click', accHandleAlertConfirm);
    }

    // Utility Functions
    function accOpenModal(id) {
      document.getElementById(id).classList.add('active');
    }

    function accCloseModal(id) {
      document.getElementById(id).classList.remove('active');
    }

    function accShowToast(message, type = 'success') {
    ...
    function accShowSubPage(pageId) {
      const accSec = document.getElementById('accountPageSection');
      if (accSec) accSec.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
      const pg = document.getElementById(pageId);
      if (pg) pg.classList.add('active');
      const accSec2 = document.getElementById('accountPageSection');
      if (accSec2) accSec2.scrollTo(0, 0);
    }

    // Theme
    function accToggleTheme() {
    ...
    // User Functions
    function accUpdateUIForUser(user) {
      const accUserProf = document.getElementById('userProfile');
      if (accUserProf) accUserProf.style.display = 'flex';
      if (user.photoURL) {
        const img = document.getElementById('userAvatarImg');
        if (img) { img.src = user.photoURL; img.style.display = 'block'; }
        const init = document.getElementById('userAvatarInitial');
        if (init) init.style.display = 'none';
        const pageImg = document.getElementById('accountPageAvatarImg');
        if (pageImg) { pageImg.src = user.photoURL; pageImg.style.display = 'block'; }
        const pageInit = document.getElementById('accountPageAvatarInitial');
        ...
        const pageInit = document.getElementById('accountPageAvatarInitial');
        if (pageInit) pageInit.textContent = initial;
      }
      const accName = document.getElementById('userName');
      if (accName) accName.textContent = user.displayName || 'User';
      const pageName = document.getElementById('accountPageName');
    ...
    function accLoadUserData(user) {
      accDb.ref(`users/${user.uid}`).once('value').then(snapshot => {
        if (snapshot.exists()) {
          ...
          if (data.lastLoginAt) {
            document.getElementById('lastLoginTime').textContent = new Date(data.lastLoginAt).toLocaleString();
          }

          if (data.settings) {
            accLoadSettings(data.settings);
          }
        }
      });
    }

    function accLoadSettings(settings) {
      ...
      document.getElementById('personalizedRecs').checked = settings.personalizedRecs ?? true;
      ...
      document.getElementById('currencySelect').value = settings.currency || 'INR';
    }

    function accSaveProfile() {
      const name = document.getElementById('profileName').value;
      const phone = document.getElementById('profilePhone').value;

      if (!name) {
        accShowToast('Please enter your name', 'error');
        return;
      }

      accDb.ref(`users/${accCurrentUser.uid}`).update({
        name,
        phone,
        updatedAt: Date.now()
      }).then(() => {
        accShowToast('Profile updated successfully', 'success');
        const accName = document.getElementById('userName');
        if (accName) accName.textContent = name;
        const pageName = document.getElementById('accountPageName');
        if (pageName) pageName.textContent = name;
        const accInit = document.getElementById('userAvatarInitial');
        if (accInit) accInit.textContent = name.charAt(0).toUpperCase();
        const pageInit = document.getElementById('accountPageAvatarInitial');
        if (pageInit) pageInit.textContent = name.charAt(0).toUpperCase();
      }).catch(error => {
        accShowToast('Failed to update profile', 'error');
      });
    }

    function accSaveSettings() {
      const settings = {
      ...
      accDb.ref(`users/${accCurrentUser.uid}/settings`).set(settings)
        .then(() => accShowToast('Settings saved', 'success'))
        .catch(() => accShowToast('Failed to save settings', 'error'));
    }

    // Address Functions
    function accShowAddressForm(address = null) {
      document.getElementById('newAddressForm').style.display = 'block';
      document.getElementById('addNewAddress').style.display = 'none';
      
      if (address) {
        accEditingAddressId = address.id;
        ...
        document.getElementById('addressType').value = address.type || 'home';
        document.querySelector('.card-title').textContent = 'Edit Address';
      } else {
        accEditingAddressId = null;
        ...
        document.getElementById('addressType').value = 'home';
        document.querySelector('.card-title').textContent = 'Add New Address';
      }
    }

    function accHideAddressForm() {
    ...
    function accSaveAddress() {
      const address = {
        ...
        userId: accCurrentUser.uid,
        updatedAt: Date.now()
      };

      if (!address.name || !address.mobile || !address.pincode || !address.city || !address.state || !address.street) {
        accShowToast('Please fill all required fields', 'error');
        return;
      }

      const promise = accEditingAddressId
        ? accDb.ref(`addresses/${accEditingAddressId}`).update(address)
        : accDb.ref('addresses').push(address);

      promise.then(() => {
        accShowToast(`Address ${accEditingAddressId ? 'updated' : 'added'} successfully`, 'success');
        accHideAddressForm();
        accLoadAddresses();
      }).catch(() => {
        accShowToast(`Failed to ${accEditingAddressId ? 'update' : 'add'} address`, 'error');
      });
    }
    ...
    function accLoadAddresses() {
      ...
      accDb.ref('addresses').orderByChild('userId').equalTo(accCurrentUser.uid).once('value')
        .then(snapshot => {
          container.innerHTML = '';
          ...
          if (!snapshot.exists()) {
            container.innerHTML = `
              <div class="empty-state">
                ...
                <div class="empty-state-title">No addresses saved</div>
                <div class="empty-state-description">Add your first address to make checkout faster</div>
              </div>
            `;
            return;
          }
          ...
          const addresses = [];
          snapshot.forEach(child => addresses.push({ id: child.key, ...child.val() }));
          ...
          addresses.forEach(address => {
            ...
            card.innerHTML = `
              ...
              <div>
                <span class="address-badge">${address.type}</span>
                ${address.isDefault ? '<span class="address-badge default">Default</span>' : ''}
              </div>
              <div class="address-actions">
                <button class="btn-outline btn-sm" onclick="editAddress('${address.id}')">Edit</button>
                <button class="btn-outline btn-sm" onclick="deleteAddress('${address.id}')">Delete</button>
                ${!address.isDefault ? `<button class="btn-sm" onclick="setDefaultAddress('${address.id}')">Set as Default</button>` : ''}
              </div>
            `;
    ...
    window.acc_editAddress = function(id) {
      accDb.ref(`addresses/${id}`).once('value').then(snapshot => {
        if (snapshot.exists()) {
          accShowAddressForm({ id, ...snapshot.val() });
        }
      });
    };

    window.acc_deleteAddress = function(id) {
      if (confirm('Are you sure you want to delete this address?')) {
        accDb.ref(`addresses/${id}`).remove()
          .then(() => {
            accShowToast('Address deleted', 'success');
            accLoadAddresses();
          })
          .catch(() => accShowToast('Failed to delete address', 'error'));
      }
    };

    window.acc_setDefaultAddress = function(id) {
      accDb.ref('addresses').orderByChild('userId').equalTo(accCurrentUser.uid).once('value')
        .then(snapshot => {
          const updates = {};
          snapshot.forEach(child => {
            updates[`addresses/${child.key}/isDefault`] = child.key === id;
          });
          return accDb.ref().update(updates);
        })
        .then(() => {
          accShowToast('Default address updated', 'success');
          accLoadAddresses();
        })
        .catch(() => accShowToast('Failed to update default address', 'error'));
    };

    // Wishlist Functions
    function accLoadWishlist() {
      const container = document.getElementById('wishlistItems');
      container.innerHTML = '<div class="spinner"></div>';

      accDb.ref('wishlist').orderByChild('userId').equalTo(accCurrentUser.uid).once('value')
        .then(snapshot => {
          container.innerHTML = '';
          ...
          if (!snapshot.exists()) {
            container.innerHTML = `
              <div class="empty-state">
                ...
                <div class="empty-state-description">Start adding products you love</div>
                <button class="btn" onclick="window.location.href='index.html'">Browse Products</button>
              </div>
            `;
            return;
          }
          ...
          snapshot.forEach(child => {
            ...
            card.className = 'product-card';
            card.innerHTML = `
              <img src="${item.image || 'https://via.placeholder.com/100'}" alt="${item.name}" class="product-image" onerror="this.src='https://via.placeholder.com/100'">
              <div class="product-details">
                <div class="product-title">${item.name || 'Product'}</div>
                <div class="product-price">₹${item.price || 0}</div>
                <div class="product-rating">
                  ${Array(5).fill().map((_, i) => `
                    <span class="star ${i < Math.floor(item.rating || 4) ? '' : 'inactive'}">★</span>
                  `).join('')}
                </div>
                <div style="display: flex; gap: 0.5rem;">
                  <button class="btn-outline btn-sm" onclick="removeFromWishlist('${child.key}')">Remove</button>
                  <button class="btn-sm" onclick="viewProduct('${item.productId}')">View</button>
                </div>
              </div>
            `;
            container.appendChild(card);
    ...
    window.acc_removeFromWishlist = function(id) {
      accDb.ref(`wishlist/${id}`).remove()
        .then(() => {
          accShowToast('Removed from wishlist', 'success');
          accLoadWishlist();
        })
        .catch(() => accShowToast('Failed to remove', 'error'));
    };

    window.acc_viewProduct = function(productId) {
      sessionStorage.setItem('viewProduct', productId);
      closeAccountPage(); setTimeout(()=>showPage('productDetailPage'),100);
    };

    // Orders Functions
    function accLoadOrders() {
      const container = document.getElementById('ordersList');
      container.innerHTML = '<div class="spinner"></div>';

      accDb.ref('orders').orderByChild('userId').equalTo(accCurrentUser.uid).once('value')
        .then(snapshot => {
          container.innerHTML = '';
          ...
          if (!snapshot.exists()) {
            container.innerHTML = `
              <div class="empty-state">
                ...
                <div class="empty-state-description">Start shopping to see your orders here</div>
                <button class="btn" onclick="window.location.href='index.html'">Shop Now</button>
              </div>
            `;
            return;
          }
          ...
          orders.sort((a, b) => (b.orderDate || b.timestamp || 0) - (a.orderDate || a.timestamp || 0));
          ...
          orders.forEach(order => {
            ...
            const date = new Date(order.orderDate || order.timestamp || Date.now()).toLocaleDateString();
            
            card.innerHTML = `
              <div class="order-header">
                <div>
                  <div class="order-id">${order.orderId || '#' + order.id.slice(-8)}</div>
                ...
                ${(order.items || []).map(item => `
                  <div class="order-item">
                    ...
                    <div style="flex: 1;">
                      ...
                      <div style="font-size: 0.875rem; color: var(--text-secondary);">Qty: ${item.quantity || 1}</div>
                      <div style="font-weight: 600; color: var(--primary);">₹${item.price || 0}</div>
                    </div>
                  </div>
                `).join('')}
              </div>
    ...
    // Login Activity
    function accShowLoginActivity() {
      accDb.ref(`loginActivity/${accCurrentUser.uid}`).once('value')
        .then(snapshot => {
          const list = document.getElementById('loginActivityList');
          list.innerHTML = '';
          
          if (!snapshot.exists()) {
            list.innerHTML = '<p style="text-align: center; padding: 2rem;">No login activity found</p>';
            return;
          }
          ...
          activities.forEach(activity => {
            ...
            item.innerHTML = `
              <div class="activity-info">
                ...
                <div class="activity-device">${activity.deviceType || 'Unknown'}</div>
              </div>
          ...
          accOpenModal('loginActivityModal');
        });
    }

    // Password Reset
    function accSendPasswordReset() {
      document.getElementById('changePasswordEmail').textContent = accCurrentUser.email;
      accAuth.sendPasswordResetEmail(accCurrentUser.email)
        .then(() => {
          accShowToast('Password reset link sent to your email', 'success');
          accCloseModal('changePasswordModal');
        })
        .catch(() => accShowToast('Failed to send reset email', 'error'));
    }

    // Alert Handlers
    function accShowAlert(title, message, onConfirm) {
      document.getElementById('alertTitle').textContent = title;
      document.getElementById('alertMessage').textContent = message;
      accAlertCallback = onConfirm;
      accOpenModal('alertModal');
    }

    function accHandleAlertConfirm() {
      if (accAlertCallback) accAlertCallback();
      accCloseModal('alertModal');
    }

    function accShowLogoutConfirmation() {
      accShowAlert('Logout Confirmation', 'Are you sure you want to logout?', () => {
        accAuth.signOut().then(() => {
          accShowToast('Logged out successfully', 'success');
    ...
    function initAccountPage() {
      if (!window._accEventsAttached) {
        window._accEventsAttached = true;
        accInitTheme();
        accSetupEventListeners();
      }
      accAuth.onAuthStateChanged(function(user) {
        if (user) {
          accCurrentUser = user;
          accUpdateUIForUser(user);
          accLoadUserData(user);
          accLoadAddresses();
          accLoadWishlist();
          accLoadOrders();
        } else {
      ...
      accShowSubPage('accountPage');
    }

    function closeAccountPage() {
      const sec = document.getElementById('accountPageSection');
    ...
    function bindVapiEvents() {
      ...
      vapiInstance.on("speech-end", () => {
        setVapiStatus("🤔 Processing your request...", "calling-state");
      });

      vapiInstance.on("message", (msg) => {
        // ---- Handle function/tool calls from the AI assistant ----
        if (msg.type === "tool-calls") {
          msg.toolCallList.forEach(tc => {
            handleAIAction(tc.function.name, JSON.parse(tc.function.arguments || "{}"));
            // Send result back to Vapi
            vapiInstance.send({
              type: "tool-call-result",
              toolCallId: tc.id,
              result: "Action executed successfully on the website."
            });
          });
        }
        ...
        if (msg.type === "transcript" && msg.role === "user") {
          setVapiStatus("You said: " + (msg.transcript || ""), "listening-state");
        }
      });
    ...
    // ---------- ACTION ENGINE — executes site navigation ----------
    function handleAIAction(actionName, params) {
      switch(actionName) {

        case "navigate_page":
        case "open_page":
          const page = (params.page || params.action || "").toLowerCase();
          if (page.includes("account") || page.includes("profile")) {
            if (typeof openAccountPage === "function") openAccountPage();
          } else if (page.includes("wishlist") || page.includes("favourite")) {
            if (typeof checkAuthAndShowPage === "function") checkAuthAndShowPage("wishlistPage");
          } else if (page.includes("order") || page.includes("track")) {
            if (typeof checkAuthAndShowPage === "function") checkAuthAndShowPage("myOrdersPage");
          } else if (page.includes("offer") || page.includes("deal")) {
            if (typeof showPage === "function") showPage("offersPage");
          } else if (page.includes("product") || page.includes("shop")) {
            if (typeof showPage === "function") showPage("productsPage");
          } else if (page.includes("home")) {
            if (typeof showPage === "function") showPage("homePage");
          } else if (page.includes("contact") || page.includes("support")) {
            if (typeof showPage === "function") showPage("homePage");
          }
          break;
        ...
        case "show_products":
          ...
          const maxPrice = params.maxPrice || params.max_price || Infinity;
          if (query) {
            // Use the site's built-in search
        ...
        case "show_contact":
          // Show contact info in transcript area
          document.getElementById("vapiTranscript").innerHTML =
            "📞 <strong>+91-9557987574</strong><br>📧 support@buyzocartshop.com";
          break;

        case "open_cart":
          if (typeof checkAuthAndShowPage === "function") checkAuthAndShowPage("cartPage");
          else if (typeof showPage === "function") showPage("cartPage");
          break;
      }
    }

    // ---------- Hint chips (text fallback if mic not available) ----------
    function vapiSendHint(text) {
      if (!vapiCallActive) {
        // Show what would happen — for demo before call starts
        document.getElementById("vapiTranscript").textContent = "You: " + text;
        setVapiStatus("Start the call and speak this command!", "");
        return;
      }
      vapiInstance.send({ type: "add-message", message: { role: "user", content: text } });
    }

    // ---------- UI helpers ----------
    function setVapiStatus(text, ringClass) {
      document.getElementById("vapiStatusText").textContent = text;
