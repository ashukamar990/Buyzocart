// Global variables
    ...
    function initColorSwitcher(product) {
      ...
      colors.forEach((color, index) => {
        ...
        colorBox.className = `color-box ${index === 0 ? 'selected' : ''} ${color.outOfStock ? 'out-of-stock' : ''}`;
        colorBox.setAttribute('data-color', color.id);
        colorBox.setAttribute('data-color-name', color.name);
        colorBox.setAttribute('data-image', color.image);
        ...
        colorBox.addEventListener('click', function() {
          ...
          document.getElementById('productDetailMainImage').style.backgroundImage = `url('${newImage}')`;
          
          // Update product images array for full screen modal
          currentProductImages = [newImage, ...colors.filter(c => c.id !== color.id).map(c => c.image)];
        });
