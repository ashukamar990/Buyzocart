...
// ============================================================
    ...
    function sanitizeURL(url) {
      ...
      const isWhitelisted = /^(https?|data:image)/i.test(url);
      if (!isRelative && !isWhitelisted) return 'about:blank';
    ...
    function renderReviews(reviewList, containerId) {
      ...
      reviewList.forEach(review => {
        ...
        if (review.fileUrl && review.fileType === 'image') {
          ...
          mediaHtml = `<div class="review-file-preview"><img src="${sUrl}" alt="Review photo" loading="lazy" class="review-img-trigger" data-full-url="${sUrl}" style="max-width:120px;max-height:120px;border-radius:8px;object-fit:cover;cursor:pointer;"></div>`;
    ...
    function setupEventListeners() {
      ...
      // Delegated click listeners for dynamic elements
      document.addEventListener('click', function(e) {
        const reviewImg = e.target.closest('.review-img-trigger');
        if (reviewImg) {
          const url = reviewImg.dataset.fullUrl;
