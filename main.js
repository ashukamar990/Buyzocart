...
// ============================================================
    ...
    function sanitizeURL(url) {
      ...
      if (!allowedProtocols.test(url)) return '';
    ...
    function renderReviews(reviewList, containerId) {
      ...
      reviewList.forEach(review => {
        ...
        const safeName = escapeHTML(review.userName || 'Customer');
        ...
        if (safeFileUrl && review.fileType === 'image') {
          mediaHtml = `<div class="review-file-preview"><img src="${safeFileUrl}" class="review-img-trigger" data-src="${safeFileUrl}" alt="Review photo" loading="lazy" style="max-width:120px;max-height:120px;border-radius:8px;object-fit:cover;cursor:pointer;"></div>`;
        ...
        reviewItem.innerHTML = `
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            ${safeUserPhoto ? `<img src="${safeUserPhoto}" loading="lazy" width="28" height="28" style="border-radius:50%;object-fit:cover;flex-shrink:0;" onerror="this.style.display='none'">` : `<div style="width:28px;height:28px;border-radius:50%;background:#e2e8f0;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;color:#64748b;flex-shrink:0;">${(safeName||'?')[0].toUpperCase()}</div>`}
            <div style="flex:1;min-width:0;">
              <span class="reviewer-name" style="font-weight:600;font-size:14px;">${safeName}</span>
    ...
    function setupEventListeners() {
      ...
      // Delegated listener for review images
      document.getElementById('reviewsList')?.addEventListener('click', function(e) {
        ...
        if (trigger) {
          ...
          if (src) window.open(sanitizeURL(src), '_blank');
        }
      });
