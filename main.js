...
// ============================================================
    ...
    function getProductImages(product) {
      ...
      if (Array.isArray(product.images) && product.images.length > 0) {
        return product.images.map(_optimize);
      }
      ...
      for (const field of possibleImageArrays) {
        if (Array.isArray(product[field]) && product[field].length > 0) {
          return product[field].map(_optimize);
        }
      }
      const possibleImageFields = ['image', 'img', 'imageUrl', 'photo', 'thumbnail', 'picture', 'url', 'mainImage', 'productImage'];
      for (const field of possibleImageFields) {
    ...
    function _scoreProduct(p, terms, originalQuery, ratingMap) {
      ...
      const rating = (ratingMap && ratingMap[p.id] !== undefined) ? ratingMap[p.id] : calculateProductRating(p.id);
      if (rating >= 4) score += 8;
    ...
    function searchProducts(query) {
      ...
      const ratingMap = bzGetRatingMap(reviews);
    ...
    function showSearchSuggestions(query) {
      ...
      topThree.forEach((product, _cardIdx) => {
        ...
        const ratingVal = (ratingMap && ratingMap[product.id] !== undefined) ? ratingMap[product.id] : calculateProductRating(product.id);
        card.innerHTML = `
     ...
     * Returns an object mapping productId -> averageRating.
     */
    function bzGetRatingMap(reviewList) {
      const stats = {};
      (reviewList || []).forEach(r => {
        if (!r.productId) return;
        if (!stats[r.productId]) stats[r.productId] = { sum: 0, count: 0 };
        stats[r.productId].sum += (r.rating || 0);
        stats[r.productId].count++;
      });
      const map = {};
      for (const pid in stats) {
        map[pid] = stats[pid].sum / stats[pid].count;
      }
      return map;
    }

    function createProductCard(product, forcedRating) {
      if (!product) {
      ...
      const rating = (forcedRating !== undefined) ? forcedRating : calculateProductRating(productId);
    ...
    function getProductScore(product, ratingMap) {
      const rating = (ratingMap && ratingMap[product.id] !== undefined)
    ...
    function renderProducts(productsToRender, containerId) {
      const container = document.getElementById(containerId);
      if (!container) return;
      // Pre-calculate ratings in O(R) instead of O(P*R)
      const ratingMap = bzGetRatingMap(reviews);
      ...
      if (needsChunking) {
        ...
        const fragment = document.createDocumentFragment();
        firstChunk.forEach(product => { if (product) fragment.appendChild(createProductCard(product, ratingMap[product.id])); });
        container.appendChild(fragment);
        ...
        function renderNextChunk() {
          ...
          chunk.forEach(product => { if (product) frag.appendChild(createProductCard(product, ratingMap[product.id])); });
        ...
        const fragment = document.createDocumentFragment();
        toRender.forEach(product => { if (product) fragment.appendChild(createProductCard(product, ratingMap[product.id])); });
        container.appendChild(fragment);
      }
