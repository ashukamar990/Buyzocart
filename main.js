// Buyzo Cart - Main Application Logic
    ...
    function getRatingMap(reviewsList = reviews) {
      ...
      const finalMap = {};
      for (const id in map) {
        finalMap[id] = map[id].sum / map[id].count;
      }
      return finalMap;
    }
    ...
    function getProductScore(product, preCalculatedRating) {
      const rating = preCalculatedRating !== undefined ? preCalculatedRating : calculateProductRating(product.id);
