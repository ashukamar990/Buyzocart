...
// ============================================================
    ...
    function _scoreProduct(p, terms, originalQuery, forcedRating) {
      ...
      const rating = (typeof forcedRating === 'number') ? forcedRating : calculateProductRating(p.id);
      if (rating >= 4) score += 8;
    ...
    /**
     * O(R) rating lookup optimization.
     * Pre-calculates average ratings for all products in a single pass.
     */
    ...
    function createProductCard(product, forcedRating) {
      ...
      const rating = (typeof forcedRating === 'number') ? forcedRating : calculateProductRating(productId);
    ...
    function getProductScore(product, forcedRating) {
      const rating = (typeof forcedRating === 'number') ? forcedRating : (function() {
    ...
    function renderProducts(productsToRender, containerId) {
      ...
      const sorted = [...(productsToRender || [])].sort((a, b) =>
        getProductScore(b, ratingMap[b.id] || 0) - getProductScore(a, ratingMap[a.id] || 0)
      );
      ...
      if (needsChunking) {
        ...
        const fragment = document.createDocumentFragment();
        firstChunk.forEach(product => { if (product) fragment.appendChild(createProductCard(product, ratingMap[product.id] || 0)); });
        container.appendChild(fragment);
        ...
        function renderNextChunk() {
          ...
          chunk.forEach(product => { if (product) frag.appendChild(createProductCard(product, ratingMap[product.id] || 0)); });
        ...
        const fragment = document.createDocumentFragment();
        toRender.forEach(product => { if (product) fragment.appendChild(createProductCard(product, ratingMap[product.id] || 0)); });
        container.appendChild(fragment);
      }
