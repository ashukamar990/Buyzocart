// Buyzo Cart - Main Application Logic
    ...
    function getRatingMap(reviewsList = reviews) {
      const map = {};
      const sums = {};
      const counts = {};
    ...
    function calculateProductRating(productId, preCalculatedRating) {
      if (preCalculatedRating !== undefined) return preCalculatedRating;
    ...
    function createProductCard(product, preCalculatedRating) {
      ...
      const rating = calculateProductRating(productId, preCalculatedRating);
    ...
    function getProductScore(product, preCalculatedRating) {
      const rating = preCalculatedRating ?? calculateProductRating(product.id);
        ...
        if (productsObj) {
          ...
          if (currentPage === 'homePage') {
            ...
            if (!trendingProducts.length) {
              trendingProducts = [...products].sort((a,b) => getProductScore(b, ratingMap[b.id] ?? 0) - getProductScore(a, ratingMap[a.id] ?? 0)).slice(0, 8);
            }
  ...
  window.renderSearchResults = function(results, query) {
    ...
    results.forEach(product => {
      ...
      if (typeof createProductCard === 'function') {
        card = createProductCard(product, ratingMap[product.id] ?? 0);
      } else {
  ...
  fnNames.forEach(fnName => {
    ...
    function showBrandProfile(brandId, brandName) {
      ...
      Promise.all([
        ...
        // Banner: check all possible Firebase field names
        var bannerUrl = bd.banner || bd.bannerUrl || bd.bannerImage || bd.coverImage || bd.cover || bd.headerImage || bd.brandBanner || bd.brandCover || '';
        var bannerStyle;
        if (bannerUrl) {
          // Real banner image
          bannerStyle = 'background:url(' + JSON.stringify(bannerUrl) + ') center/cover no-repeat;';
        } else if (logo) {
          // Auto-generate: blurred logo as banner background
          bannerStyle = 'background:url(' + JSON.stringify(logo) + ') center/cover no-repeat;position:relative;';
        } else {
          // Fallback: color gradient
          bannerStyle = 'background:linear-gradient(135deg,' + color + 'ee,' + color + '88);';
        }
        // Add blur overlay for logo-as-banner
        var bannerOverlay = (!bannerUrl && logo) ? '<div style="position:absolute;inset:0;backdrop-filter:blur(18px) brightness(.7);-webkit-backdrop-filter:blur(18px) brightness(.7);border-radius:0;"></div>' : '';
        ...
        page.innerHTML =
          '<div style="max-width:640px;margin:0 auto;">'
            ...
            + '<div style="position:relative;padding-bottom:36px;">'
              + '<div style="height:130px;' + bannerStyle + 'border-radius:0;position:relative;overflow:hidden;">'
              + bannerOverlay
              + '</div>'
        ...
        if (brandProds.length) {
          ...
          setTimeout(function() {
            ...
            const brandRatingMap = getRatingMap();
            var popular = brandProds.slice().sort(function(a,b) {
              var ra = brandRatingMap[a.id] ?? 0;
              var rb = brandRatingMap[b.id] ?? 0;
              return rb - ra;
            }).slice(0, 4);
