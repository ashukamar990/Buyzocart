...
// ============================================================
    ...
    function showProductDetail(product) {
      ...
      if (freshProduct.brand) {
        ...
        const safeBrandName = escapeHTML(freshProduct.brand);
        brandBadgeEl.innerHTML = `<div style="display:inline-flex;align-items:center;gap:5px;background:#eff6ff;color:#2563eb;padding:5px 14px;border-radius:20px;font-size:12px;font-weight:700;margin:6px 0 8px;cursor:pointer;border:1px solid #bfdbfe;">🏷️ ${safeBrandName}${blueTick}</div>`;
        const innerDiv = brandBadgeEl.querySelector('div');
        if (innerDiv) innerDiv.onclick = () => showBrandProfile(bBrandId, freshProduct.brand);
    ...
    function renderReviews(reviewList, containerId) {
      ...
      reviewList.forEach(review => {
        ...
        if (review.fileUrl && review.fileType === 'image') {
          ...
          mediaHtml = `<div class="review-file-preview"><img src="${safeFileUrl}" alt="Review photo" loading="lazy" style="max-width:120px;max-height:120px;border-radius:8px;object-fit:cover;cursor:pointer;" onclick="window.open('${safeFileUrl}','_blank')"></div>`;
        ...
        if (review.youtubeUrl) {
          ...
          const safeYoutubeUrl = escapeHTML(sanitizeURL(review.youtubeUrl));
        ...
        const safeUserPhoto = escapeHTML(sanitizeURL(review.userPhoto));
