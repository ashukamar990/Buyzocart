// Buyzo Cart - Main Application Logic
    ...
    /**
     * Pre-calculates product ratings in a single pass.
     * Optimization: Reduces complexity from O(P * R) to O(P + R) during rendering.
     */
    function getRatingMap(reviewsList) {
      ...
      reviewsList.forEach(r => {
        if (r.productId) {
          ...
          map[r.productId] += r.rating || 0;
          counts[r.productId]++;
        }
      });
      const finalMap = {};
      for (const id in map) {
        finalMap[id] = map[id] / counts[id];
      }
      return finalMap;
    }

    /**
     * Calculates or looks up the average rating for a product.
     * @param {string} productId - The product ID.
     * @param {Object} [ratingMap=null] - Optional pre-calculated map for O(1) lookup.
     */
    ...
    // ── Product score for smart sorting (orders × weight + rating × weight) ──
    // Optimization: Uses pre-calculated ratingMap to ensure O(1) rating lookup during sort.
