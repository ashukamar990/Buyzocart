...
// ============================================================
    ...
    function sanitizeURL(url) {
      if (!url) return "";
      // Strip dangerous characters and only allow safe protocols
    ...
    function createProductCard(product) {
      ...
      const productImage = sanitizeURL(getProductImage(product));
      ...
      const _cardBrandLogo = sanitizeURL(product.brandLogo || product.brandIcon || '');
    ...
    function showProductDetail(product) {
      ...
      if (freshProduct.brand) {
        ...
        brandBadgeEl.innerHTML = `<div class="brand-trigger" data-brand-id="${bBrandId}" data-brand-name="${escapeHTML(freshProduct.brand)}" style="display:inline-flex;align-items:center;gap:5px;background:#eff6ff;color:#2563eb;padding:5px 14px;border-radius:20px;font-size:12px;font-weight:700;margin:6px 0 8px;cursor:pointer;border:1px solid #bfdbfe;">🏷️ ${escapeHTML(freshProduct.brand)}${blueTick}</div>`;
    ...
    function renderReviews(reviewList, containerId) {
      ...
      reviewList.forEach(review => {
        ...
        if (review.fileUrl && review.fileType === 'image') {
          mediaHtml = `<div class="review-file-preview"><img src="${sanitizeURL(review.fileUrl)}" alt="Review photo" loading="lazy" class="review-img-trigger" data-url="${sanitizeURL(review.fileUrl)}" style="max-width:120px;max-height:120px;border-radius:8px;object-fit:cover;cursor:pointer;"></div>`;
        ...
        if (review.youtubeUrl) {
          ...
          if (ytId) mediaHtml += `<div style="margin-top:8px;"><a href="${sanitizeURL(review.youtubeUrl)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:6px;background:#fee2e2;color:#dc2626;padding:6px 12px;border-radius:20px;font-size:12px;font-weight:600;text-decoration:none;">▶ Watch Video Review</a></div>`;
        }

        reviewItem.innerHTML = `
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            ${review.userPhoto ? `<img src="${sanitizeURL(review.userPhoto)}" loading="lazy" width="28" height="28" style="border-radius:50%;object-fit:cover;flex-shrink:0;" onerror="this.style.display=\'none\'">` : `<div style="width:28px;height:28px;border-radius:50%;background:#e2e8f0;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;color:#64748b;flex-shrink:0;">${escapeHTML((review.userName||'?')[0].toUpperCase())}</div>`}
            <div style="flex:1;min-width:0;">
    ...
    function setupEventListeners() {
      ...
      document.addEventListener('click', (e) => {
        // Brand trigger from createProductCard
        const brandTrigger = e.target.closest('.brand-trigger');
        if (brandTrigger) {
          e.stopPropagation();
          const bid = brandTrigger.getAttribute('data-brand-id');
          const bname = brandTrigger.getAttribute('data-brand-name');
          if (typeof showBrandProfile === 'function') {
            showBrandProfile(bid, bname);
          }
          return;
        }

        // Review image trigger from renderReviews
        const reviewImgTrigger = e.target.closest('.review-img-trigger');
        if (reviewImgTrigger) {
          const url = reviewImgTrigger.getAttribute('data-url');
